...
- 팝업 헤더의 새로 고침 버튼으로 모든 provider를 즉시 다시 조회합니다.
- 핀 버튼으로 팝업을 고정합니다.
...
- **프로바이더**: provider별 사용 여부, 갱신 주기, secondary 행, credits 행, Codex 명령, Claude 자격증명 상태, Claude 세션 자동 갱신, 재연결
...
- Claude 세션 자동 갱신은 기본으로 켜져 있습니다. 끄면 앱이 백그라운드에서 `claude`를 실행하지 않으며, 8시간 토큰이 만료된 뒤에는 로그인 또는 재연결 전까지 사용량 표시가 멈춥니다.
